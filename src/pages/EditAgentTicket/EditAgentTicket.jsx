...
function EditAgentTicket() {
    ...
    const fetchTicket = async () => {
      try {
        ...
        const res = await fetch(
          `${API_BASE_URL}/api/agent/ticket/${ticketId}`,
          {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const res = await fetch(
        `${API_BASE_URL}/api/agent/tickets/${ticketId}`,
        {
          method: "PATCH",
          headers: {
      ...
      navigate("/agent-assigned-ticket");
    } catch (err) {
...
export default EditAgentTicket;
