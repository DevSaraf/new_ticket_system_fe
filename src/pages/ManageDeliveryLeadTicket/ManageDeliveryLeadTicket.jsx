...
function ManageDeliveryLeadTicket() {
  ...
  const fetchTickets = async () => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/delivery-lead/tickets`,
        {
          method: "GET",
          headers: {
  ...
  const fetchAgents = async () => {
    try {
      ...
      const response = await fetch(
        "`${API_BASE_URL}/api/delivery-lead/users/agents`",
        {
          method: "GET",
          headers: {
            Authorization: token, // Correctly format the Authorization header
      ...
      // Log the data to ensure it is being fetched correctly
      console.log("Agents data:", result);
  ...
  const assignTicket = async (ticketId, assigneeEmail) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/delivery-lead/ticket/${ticketId}/assign`,
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            assignee_email: assigneeEmail, // agent OR DL
          }),
        }
      );
  ...
  return (
    <div className="container-fluid">
      <div className="d-sm-flex align-items-center mb-4">
        <h1 className="h4 mb-0 text-gray-800">UnAssigned Ticket</h1>
      </div>
...
export default ManageDeliveryLeadTicket;
